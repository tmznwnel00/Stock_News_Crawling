...
class NewsApp(QWidget):
    ...
    def fetch_news_for_keyword_daum(self, kw):
        header = {
        ...
        try:
            ...
            for a in articles:
                ...
                ts = self.parse_time(time_text)
                news_items.append((media, ts, kw, title, link))
        except Exception as e:
            print(f"Error loading Daum news for {kw}: {e}")
        return news_items
    ...
    def load_news(self):
        if not self.keywords:
            ...
            return  # 키워드가 없으면 크롤링하지 않음

        new_results = []
        new_links = set()
        with ThreadPoolExecutor(max_workers=min(8, len(self.keywords))) as executor:
            daum_futures = [executor.submit(self.fetch_news_for_keyword_daum, kw) for kw in self.keywords]
            for future in daum_futures:
